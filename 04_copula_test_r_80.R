...
mydata=read.csv(file="test_80er.csv",head=TRUE,sep=";")
...
# double check!
clayton <- claytonCopula(param = 1)
set.seed(500)
m <- pobs(as.matrix(mydata))
fit <- fitCopula(clayton,m,method="itau")
coef(fit)
...
u <- rCopula(3965,claytonCopula(par))
...
# again looks more like a gamma, lnorm or Weibull, or maybe exponential 
...
fit2_exponentialkids <- fitdist(mydata.1$kidsincome, "exp", method = "mme")
...
ggplot(data = mydata.1) +
  ...
  geom_line(aes(x=mydata.1$kidsincome, y=dexp(mydata.1$kidsincome,fit2_exponentialkids$estimate[1]), col="exponential distribution"), alpha = 0.7, size = 1) + 
...
ggsave("kidsdist_80.pdf")
## here choose gamma (pretty uneven fit)!
...
# Compute the density
pdf_mvd <- dMvdc(v, my_dist_80)
# Compute the CDF
cdf_mvd <- pMvdc(v, my_dist_80)
...
persp(my_dist_80, dMvdc, xlim = c(0, 50000), ylim=c(0, 50000), main = "Density")
contour(my_dist_80, dMvdc, xlim = c(0, 50000), ylim=c(0, 50000), main = "Contour plot")
persp(my_dist_80, pMvdc, xlim = c(0, 50000), ylim=c(0, 50000), main = "CDF")
contour(my_dist_80, pMvdc, xlim = c(0, 50000), ylim=c(0, 50000), main = "Contour plot")
